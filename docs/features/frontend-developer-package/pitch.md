...
- Do not copy Anthropic or OpenAI skill wording where redistribution rights are
  unclear. Author original guidance from verified principles and record any
  permissively licensed adapted material with a pinned notice.
...
- Anthropic's official skill and blog establish the anti-generic design goal and
  progressive-disclosure approach, but `DESIGN.md` is not its canonical
  workflow. The official Claude Code repository does not provide a clear
  permissive grant for republishing that skill, so this package will not copy
  it:
...
The parent owns product scope, package composition, original design guidance,
provider-boundary decisions, vertical slices, and final pitch and plan
...
The requested execution preference is **accept-all implementation**. This is a
preference only until the complete plan is approved. Even after plan approval,
implementation must pause for setup, test, check, commit, or publication
failure; material review findings; material forecast variance; credential or
privacy decisions; changed scope, delivery boundaries, dependencies, or
authority.
