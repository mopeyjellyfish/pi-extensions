...
The installed Claude Code plugin adds useful rigor through a preflight check,
five parallel review lenses, history and prior-review context, independent issue
confidence scoring, false-positive filtering, and a final boundary check. Its
child agents orchestrate more children, which does not fit this repository.
Instead, the parent can launch named, disjoint, read-only lanes in parallel and
join them before a decision. No child receives fan-out authority.
...
the intended model. The user also decided that review results stay in Pi and
that `/code-review` replaces `/review-change`.

## Proposed solution

Add one `/code-review [target] [--comments] [--fix]` prompt template to the
...
The default result stays in Pi. `--comments` grants one bounded GitHub mutation
for an eligible pull request: after the final boundary check, use the installed
GitHub method to submit one review containing inline comments for retained issues
on changed lines. Do not post when no issue survives. A missing authenticated
GitHub capability reports the unmet action without a substitute.
...
When `--comments` and `--fix` are both present, fix locally and defer all GitHub
...
- Without `--comments` or `--fix`, do not post, edit, commit, push, or perform any
  other repository mutation.
- `--comments` authorizes one eligible GitHub review only. It does not authorize
  edits, issue changes, merge, release, or other remote actions.
...
- Claude Code's official command uses several parallel Sonnet reviewers and one
  Haiku scorer per candidate. The accepted Pi topology uses five parallel
  Sol-medium Reviewer lenses and one consolidated Luna-medium scorer under an
  Astra-high parent. This preserves independent lenses and scoring with fewer
  child launches, but one consolidated scorer has less isolation than one scorer
  per issue.
...
- Inline review comments are public remote mutations. `--comments` must be the
...
- The local Claude Code command and README disagree about whether it launches four
  or five primary review agents. This feature follows the command's five named
  lenses, not the README count.
...
- **Fixed document:** Draft revision with SHA-256
  `087d7b6164e4494f0e6cdeaeb23b67e97e4f73fce5562abd842cf1436a3bb864`.
- **Status:** Replacement review `309e7374` approved the complete revision with
  no blockers, material questions, or recommendations. It verified that the
  narrow Reviewer lens mode resolves review `8660b21d`'s scope conflict without
  weakening ordinary integrated review.
...
Engineering skill, prompt, package documentation, root `agents/reviewer.md`,
aggregate documentation, and required feature documents. It does not authorize
merge, release, deployment, destructive cleanup, or unrelated changes.

At command runtime, no flag means read-only review. `--comments` is explicit
authority for one bounded eligible GitHub review. `--fix` is explicit authority
for local isolated repairs and required verification only. Combined flags fix
locally and defer comments. Neither flag grants commit, push, merge, release,
deployment, cleanup, or unrelated authority.
...
- **AC-001 — Command surface:** The Engineering package exposes
  `/code-review [target] [--comments] [--fix]` and no longer exposes
  `/review-change`.
...
- **AC-010 — Inline comments:** With `--comments` only, an authenticated eligible
  GitHub pull request and retained findings produce one review with inline
  comments on changed lines after a final head check. No retained finding means
  no remote comment. Missing GitHub capability is reported without substitution.
- **AC-011 — Local fixes:** With `--fix`, one configured Worker repairs only
  ...
  push.
- **AC-012 — Combined flags:** With `--comments --fix`, the command fixes locally,
