...
- **AC-014 — Attribution:** every adapted Apache-2.0 reference ships with the required license, retained notice material, upstream commit pin, and collective package modification notice; package documentation identifies the derivative boundary without repeating per-reference provenance banners.
...
- **AC-017 — Explicit frontend delivery:** when material UI work is in scope and the capabilities are installed, Shape explicitly uses `frontend-design`, planning names the applicable frontend build and proof skills for each interface slice, and `developing-changes` and `implement` preserve that route through implementation and visual validation. Conditional fallbacks preserve independently installable packages, and the private fixed Worker exposes only the implementation-time frontend skills needed to honor the accepted route.
