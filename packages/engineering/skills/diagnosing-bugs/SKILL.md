...
These local requirements apply whenever this skill runs in Pi.
...
Before designing the Phase 1 loop, use the `question` tool to collect every
missing input that would materially affect the loop. Ask one batch of at most
four focused questions, skipping facts already supplied. Cover these subjects
when they are unknown:

1. the exact expected and actual observable symptom;
2. the fastest known reproduction, command, test, URL, or user action;
3. whether it is deterministic, intermittent, or a performance regression,
   including the first known good or recently changed state; and
4. the environment, access, and redacted artifact that can make the failure
   observable.

Give each question concrete choices and rely on the tool's built-in free-text
answer for details. Never add fake `Other`, `Chat about this`, `Next`, or
`Submit` choices. If the user redirects the question, address that clarification
and call `question` again with its continuation ID. If the tool is unavailable
or the user cancels, state the exact missing details and wait; do not invent a
reproduction or move to hypotheses.

### Human-in-the-loop loop

Use `scripts/hitl-loop.template.sh` only when the loop requires a person to
perform an action. Copy it to a temporary, ignored location and edit the copy;
never change the shipped template. Run the copy through `bash`, ask the user to
perform only the necessary action, and feed its captured observations into the
smallest repeatable loop. Prefer an agent-runnable loop as soon as one is
possible.

### Repair loop

When Phase 5 begins, load and follow the `test-driven-development` skill. Keep
