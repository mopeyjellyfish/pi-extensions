...
function normalizeEdits(edits: readonly Edit[]): unknown[] {
  return edits.map((edit) => {
    if (edit.kind === "insert")
      return { kind: edit.kind, cursor: edit.cursor, text: edit.text, mode: edit.mode };
...
describe("CUT N*", () => {
  ...
  it("resolveBlockEdits expands a cut block into capture and deletes", () => {
    ...
    expect(resolved.flatMap((edit) => (edit.kind === "delete" ? [edit.anchor.line] : []))).toEqual([
      2, 3,
    ]);
  });
  ...
  it("lowers an unresolvable blank-line anchor to plain `PUT > N:` instead of failing", () => {
    ...
    expect(
      result.warnings?.some((w) =>
        /could not resolve a syntactic block.*applied as plain `PUT >2:`/.test(w),
      ),
    ).toBe(true);
  });
