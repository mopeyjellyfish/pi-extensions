 ...
 */
import { HL_CUT_KEYWORD, HL_PUT_KEYWORD, HL_RANGE_SEP } from "./format.ts";
import {
...
export function hasClipboardEdit(edits: readonly Edit[]): boolean {
  return edits.some(
    (edit) =>
      ...
      edit.kind === "paste" ||
      (edit.kind === "block" &&
        (edit.mode === "cut" || edit.mode === "paste_after" || edit.register !== undefined)),
  );
}
...
export function forkClipboard(source?: Clipboard): Clipboard {
  ...
  return {
    ...
    ...(source.pendingAnonCuts === undefined
      ? {}
      : { pendingAnonCuts: [...source.pendingAnonCuts] }),
  };
}
