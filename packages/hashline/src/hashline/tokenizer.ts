 ...
 * the shape.
 */
function finishTargetScan(
  line: string,
  index: number,
  end: number,
  target: RegisterableTarget,
): TargetScan {
...
function scanPutSpanTarget(line: string, cursor: number, end: number): TargetScan | null {
  ...
  if (next < end && codeAt(line, next) === CHAR_STAR) {
    ...
    return finishTargetScan(line, next + 1, end, {
      kind: "block",
      anchor: { line: range.range.start.line },
    });
  }
...
function classifyLine(line: string, lineNum: number): Token {
  ...
  if (isHunkLead) {
    ...
    if (hunk !== null)
      return { kind: "op-block", lineNum, target: hunk.target, hadColon: hunk.hadColon };
  }
