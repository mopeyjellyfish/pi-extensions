...
const RANGE_OP_FORMS: Record<
  AbsoluteRangeOp,
  {
    single: (line: number, reg?: string) => string;
    range: (start: number, end: number, reg?: string) => string;
  }
> = {
  replace: {
...
export function invalidAbsoluteRangeMessage(
  ...
  const counted =
    Number.isSafeInteger(countedEnd) && countedEnd >= start
      ? forms.range(start, countedEnd, register)
      : null;
 ...
 */
export function textualBoundaryEchoWarning(
  startLine: number,
  leading: number,
  trailing: number,
): string {
 ...
 */
export function boundaryVariantRepairWarning(
  startLine: number,
  kept: number,
  dropped: number,
): string {
...
export function emptyRegisterPasteWarning(name: string, known: readonly string[]): string {
  ...
  return known.length === 0
    ? base
    : `${base} Available registers: ${known.map((k) => `\`@${k}\``).join(", ")}.`;
}
 ...
 */
export function blockSingleLineMessage(
  line: number,
  op: BlockOp,
  enclosingBlock?: BlockSpan,
): string {
