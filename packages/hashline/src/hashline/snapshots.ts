...
class BoundedLruMap<Key, Value> {
  ...
  constructor(maxEntries: number, maxSize: number, sizeOf: (value: Value) => number) {
  ...
  set(key: Key, value: Value): void {
    this.#values.delete(key);
    this.#values.set(key, value);
    while (this.#values.size > this.#maxEntries || this.size() > this.#maxSize) {
  ...
  private size(): number {
    let total = 0;
    for (const value of this.#values.values()) total += this.#sizeOf(value);
    return total;
  }
}
...
export class InMemorySnapshotStore extends SnapshotStore {
  readonly #versions: BoundedLruMap<string, Snapshot[]>;
  readonly #maxVersionsPerPath: number;

  constructor(options: InMemorySnapshotStoreOptions = {}) {
    super();
    this.#versions = new BoundedLruMap<string, Snapshot[]>(
      options.maxPaths ?? DEFAULT_MAX_PATHS,
      options.maxTotalBytes ?? DEFAULT_MAX_TOTAL_BYTES,
      (history) => history.reduce((total, version) => total + version.text.length, 1),
    );
