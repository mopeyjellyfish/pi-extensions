...
If the defect changes the pitch materially, rewind to Pitch rather than smuggling product scope into a bug fix. If it reveals work inside the active slice, add that discovered work to Todo. If a stated stop condition is reached or the shaped agent investment is exceeded before the timer, stop and ask the user to cut scope, rewind, or abandon. After a cut, request the evidence-gated Build transition without another approval ceremony. Use circuit commands only after the wall-clock backstop expires.

Method influence: Matt Pocock's [diagnosing-bugs skill](https://github.com/mattpocock/skills/blob/d574778f94cf620fcc8ce741584093bc650a61d3/skills/engineering/diagnosing-bugs/SKILL.md), adapted to this repository's RED/GREEN ledger and review gates. Pocock requires cleanup and a post-mortem before declaring done. This workflow likewise requires removing probes and recording root-cause evidence, but omits a separate post-mortem artifact to avoid duplicating the ledger and review evidence.
