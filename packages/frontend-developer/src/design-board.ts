...
function html(board: Board): string {
  ...
  const directions = board.directions
    .map((direction) => {
      ...
      const recommended =
        direction.id === snapshot.recommendedDirectionId
          ? '<span class="recommended">Recommended</span>'
          : "";
      const description = direction.description
        ? `<span class="description">${escapeHtml(direction.description)}</span>`
        : "";
      return `<label class="direction"><img alt="" src="${snapshot.path}image/${encodeURIComponent(direction.id)}"><span class="choice"><input${checked} name="directionId" required type="radio" value="${escapeHtml(direction.id)}"><strong>${escapeHtml(direction.label)}</strong>${recommended}</span>${description}</label>`;
    })
    .join("\n");
...
:root{color-scheme:dark;font-family:ui-sans-serif,system-ui,sans-serif;background:#0b0d10;color:#f4f5f7}*{box-sizing:border-box}body{margin:0;background:radial-gradient(circle at top,#1a2029 0,#0b0d10 42rem)}main{width:min(1180px,calc(100% - 32px));margin:0 auto;padding:48px 0 80px}header{display:flex;align-items:end;justify-content:space-between;gap:24px;margin-bottom:28px}h1{font-size:clamp(2rem,5vw,4.25rem);letter-spacing:-.055em;line-height:.95;margin:0;max-width:14ch}header p{color:#aab1bd;max-width:36rem;margin:0}.site{color:#b9ff66}.directions{border:0;padding:0;margin:0;display:grid;grid-template-columns:repeat(auto-fit,minmax(min(100%,260px),1fr));gap:18px}.direction{display:flex;flex-direction:column;gap:14px;padding:14px;border:1px solid #ffffff1a;border-radius:18px;background:#12161c;cursor:pointer}.direction:has(input:checked){border-color:#b9ff66;box-shadow:0 0 0 3px #b9ff6626}.direction img{width:100%;aspect-ratio:4/3;object-fit:cover;border-radius:10px;background:#080a0d}.choice{display:flex;align-items:center;gap:9px}.choice input{width:20px;height:20px;accent-color:#b9ff66}.recommended{margin-left:auto;color:#0b0d10;background:#b9ff66;border-radius:999px;padding:4px 8px;font-size:.72rem;font-weight:800;text-transform:uppercase}.direction .description{color:#aab1bd;line-height:1.5}.feedback{display:grid;gap:10px;margin-top:24px;padding:20px;border-radius:18px;background:#e9edf2;color:#101318}.feedback textarea{width:100%;min-height:110px;resize:vertical;border:1px solid #aab2bd;border-radius:10px;padding:12px;font:inherit}.feedback button{justify-self:start;border:0;border-radius:999px;padding:11px 18px;background:#101318;color:#fff;font-weight:750;cursor:pointer}.feedback button:focus-visible,.site:focus-visible,input:focus-visible,textarea:focus-visible{outline:3px solid #73bfff;outline-offset:3px}.saved{color:#276500;font-weight:700}@media(max-width:700px){main{padding-top:28px}header{align-items:start;flex-direction:column}h1{font-size:2.7rem}.recommended{font-size:.62rem}}
</style></head><body><main><header><div><h1>${escapeHtml(snapshot.title)}</h1></div><p>Compare every image-backed direction, choose one, and explain what should carry forward. ${site}</p></header><form method="post" action="${snapshot.path}feedback"><input name="token" type="hidden" value="${snapshot.token}"><input name="version" type="hidden" value="${String(snapshot.version)}"><fieldset class="directions"><legend>Design directions</legend>${directions}</fieldset><section class="feedback"><label for="notes"><strong>What do you like, and what should change?</strong></label><textarea id="notes" maxlength="${String(MAX_NOTES_LENGTH)}" minlength="1" name="notes" required>${notes}</textarea><button type="submit">Submit feedback</button>${prior}</section></form></main></body></html>`;
}
