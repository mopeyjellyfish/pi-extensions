...
This package vendors the preceding Vercel React View Transitions guide,
including its complete `references/` directory, from
`vercel-labs/agent-skills` commit
`063bee94c3f4df8453406c830b0a7df0f2860278` under MIT.

Use it only for applicable React view-transition animation work. Target repository instructions,
installed React and framework versions, existing dependencies, and measured
browser evidence override generic guidance.
Third-party library examples are optional and do not install dependencies
automatically.
