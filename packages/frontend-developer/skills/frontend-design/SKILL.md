...
- For a focused request to evaluate, refine, enhance, fix, extract, document,
  or iterate on product UI, load `interface-craft` and select one operation.
  Audits and critiques remain evaluation-only unless a follow-on is requested.
